...
void c_custom::tab_area(const char* str_id, ImVec2 size, std::function < void() > content) {
    ...
    GetWindowDrawList()->AddRectFilled(GetCurrentWindow()->Pos, GetCurrentWindow()->Pos + GetCurrentWindow()->Size, ImColor(10, 10, 10), GetStyle().WindowRounding, ImDrawFlags_RoundCornersTopLeft);
...
void c_custom::begin_child(const char* name, ImVec2 size) {
    ...
    GetWindowDrawList()->AddRectFilled(pos, pos + size, to_vec4(10, 10, 10, custom.m_anim), 4);
